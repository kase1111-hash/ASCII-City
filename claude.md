...
**Current Status**: Subsystems are built and tested in isolation; the project is in a "refocusing" phase to wire behavioral circuits, NPC intelligence, and full memory into the game loop. See `REFOCUS_PLAN.md` for the integration roadmap.
...
```
src/shadowengine/           # Main source (88 active Python files)
├── game.py                 # Main game loop (272 lines)
├── command_handler.py      # Command routing (557 lines)
├── location_manager.py     # Location generation (398 lines)
├── conversation.py         # NPC dialogue management (309 lines)
├── world_state.py          # State consistency for LLM
├── config.py               # Game & theme configuration
├── signal_router.py        # Signal routing for behavioral circuits
├── event_bridge.py         # Game event integration
...
├── generation/             # Procedural location/dialogue generation
├── npc_intelligence/       # Rumor networks, social dynamics
├── inspection/             # Zoom inspection system
├── moral/                  # Moral consequence tracking
├── ui/                     # Help system, tutorial, command history
├── scenarios/              # Built-in game scenarios
└── _deferred/              # Parked subsystems (not in game loop)
    ├── audio/              # Audio & TTS (12 files)
    ├── studio/             # ASCII Art Studio (12 files)
    ├── voice/              # Speech-to-text (6 files)
    ├── modding/            # Extensibility (6 files)
    └── replay/             # Deterministic replay (3 files)

tests/                      # 66 active test files mirroring src/ structure
docs/                       # Architecture, design, roadmap docs
docs/modules/               # 12 detailed module specifications
docs/specs/                 # 11 technical specification documents
```
...
- `main.py` - Entry point
- `src/shadowengine/game.py` - Main game loop (272 lines, delegates to handlers)
...
- `docs/modules/` - 12 detailed module specifications
- `docs/specs/SPEC_*.md` - Technical specifications (11 files)
