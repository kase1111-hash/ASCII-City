...
class SourceType(Enum):
    """Provenance of a memory entry — tracks where content originated."""
    SYSTEM = "system"           # Established by game setup / narrative spine
    LLM_GENERATED = "llm"      # Generated by LLM (locations, dialogue, etc.)
    PLAYER_ACTION = "player"   # Result of a player action or command
    UNKNOWN = "unknown"        # Legacy / untagged entries
...
class Event:
    ...
    source_type: SourceType = SourceType.UNKNOWN         # Provenance tracking
    ...
    def from_dict(cls, data: dict) -> 'Event':
        ...
        # Handle legacy save files without source_type
        source_raw = data.pop("source_type", "unknown")
        try:
            source = SourceType(source_raw)
        except ValueError:
            source = SourceType.UNKNOWN
        return cls(**data, source_type=source)
